...
(function(){
  ...
  function buildPreviewMarkup(proj){
    var sectionsHtml = proj.sections.map(function(sec){
      var swatchAlt = function(i){ return proj.name + ' \u2014 ' + sec.label + ' concept photo ' + i; };
      return '<div class="mp-section">' +
        '<h4>' + sec.label + '</h4>' +
        '<div class="mp-grid">' +
          '<div class="mp-swatch">' + mediaHTML(sec.imgs[0], swatchAlt(1), sec.grad[0]) + '</div>' +
          '<div class="mp-swatch">' + mediaHTML(sec.imgs[1], swatchAlt(2), sec.grad[1]) + '</div>' +
          '<div class="mp-swatch">' + mediaHTML(sec.imgs[2], swatchAlt(3), sec.grad[2]) + '</div>' +
        '</div>' +
      '</div>';
    }).join("");
